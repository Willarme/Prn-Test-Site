...
import { useLocalRequestBudgetFixtures } from './helpers/request-budget-fixture';
...
function governed(raw: unknown, enabled = true, failure?: ModelCallResult, cap?: number) {
  ...
  return { calls, read: (evidence_id?: string) => readEquipmentLabel({ bytes: JPEG, mime: "image/jpeg", tenant_id: "prn", request_id: "rq_capacity_synthetic", ...(evidence_id ? { evidence_id } : {}) }, { deps }) };
}
