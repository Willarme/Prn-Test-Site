...
describe("migration 00012 — applied 2026-08-25, and structurally sound", () => {
  ...
  it("keeps 00012 and every migration outside the documented branch reservation gap", () => {
    ...
    // Stage 13 adds 00027 feature states; 00016-00018 remain the only gap.
    expect(numbers).toEqual([...Array.from({ length: 15 }, (_, i) => i + 1), 19, 20, 21, 22, 23, 24, 25, 26, 27]);
    expect(migrations).toContain("00026_request_media_budget.sql");
    expect(migrations).toContain("00027_feature_states.sql");
