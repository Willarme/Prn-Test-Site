...
type SequenceGrant = { relname: string; acl: string; authenticated_usage: boolean; authenticated_select: boolean };
...
describe("loop migration sequence permission scope", () => {
  it("grants only its two new identity sequences while preserving every existing sequence ACL", async () => {
    ...
    try {
      ...
      const before = (await db.query<SequenceGrant>(sequenceGrants)).rows;
      ...
      const after = (await db.query<SequenceGrant>(sequenceGrants)).rows;
