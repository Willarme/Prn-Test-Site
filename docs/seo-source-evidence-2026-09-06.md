...
`content/source-evidence/ac-blowing-warm-air.json` contains the reviewed public-source capture metadata and exact claim assessments from September 6, 2026. All eleven frozen URLs returned actual article responses. One full claim is supported: the repair range and relative examples attributed to Carrier. Six claims have partial support and three have no source binding. Three further source conflicts or coverage gaps concern continued operation, categorical room-scope language, and a repair-cost/mechanism comparison.
...
A06 consumes only positive exact support while retaining all three additional review findings as blockers. Runtime capability and launch receipts remain absent. The previous generated SEO draft remains rejected. Illustrated services and prepared demo results are concept/fixture evidence, not proof of live capability.
...
The browser checks cover320–2560px, three landscape sizes, no-JS, reduced motion and deliberately caught initialization exceptions. Five actual FAQ answers exist in frozen v43; earlier notes counting six were incorrect. A confirmed320px Repair Record methodology link overflows by31px because it does not wrap. No page CSS was changed. The full form/image/packet action sequence, complete accessibility and Core Web Vitals, actual production capability outcomes and the second-machine fidelity result remain separate open requirements.
