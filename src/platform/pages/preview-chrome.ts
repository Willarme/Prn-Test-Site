import { arrangePreviewForMobile, PREVIEW_RESPONSIVE_CSS } from "./preview-responsive";
import { CUSTOMER_SHELL_CSS, renderCustomerHeader, renderCustomerFooter } from "./customer-shell";
 ...
 * receive styling. Narrow layouts stack the annotated comparison panels;
 * source style blocks, card artwork, wording and handlers pass through unchanged.
...
export function addPreviewChrome(source: string, stage: string, snapshot: FeatureSnapshot): string {
  ...
  const acGuide = featureHref(snapshot, "/problems/ac-blowing-warm-air") ? '<a class="prn-preview-ac" href="/problems/ac-blowing-warm-air">AC guide</a>' : "";
  const chrome = `<nav data-preview-chrome="navigation" aria-label="Current product"><div class="prn-preview-chrome-inner"><span class="prn-preview-stage" aria-current="page"><i class="prn-preview-mark" aria-hidden="true"></i>${STAGES[stage]}</span><div class="prn-preview-links">${acGuide}</div></div></nav>`;
  ...
  const html = arrangePreviewForMobile(annotateOriginalNavigation(source, stage), stage).replace(/<aside\b[^>]*\bdata-preview-scope\b[^>]*>/i, tag => {
    const withoutStyle = tag.replace(/\sstyle=(['"])[\s\S]*?\1/i, "");
    return withoutStyle.replace(/>$/, ` style='${SCOPE_STYLE}'>`);
  });
  return html.replace(/<\/head>/i, `<style data-preview-chrome-style>${CHROME_CSS}${PREVIEW_RESPONSIVE_CSS}${CUSTOMER_SHELL_CSS}</style>$&`)
    .replace(/<body\b[^>]*>/i, `$&${renderCustomerHeader(snapshot)}${chrome}`)
    .replace(/<\/body>/i, `${renderCustomerFooter(snapshot)}$&`);
}
