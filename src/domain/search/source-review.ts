...
export function evaluateSourceReview(input: unknown, now = new Date()) {
  ...
  if (bundle_sha256 !== sourceReviewHash(payload) || bundle.binding_sha256 !== REVIEWED_BINDING ||
      binding_sha256 !== REVIEWED_BINDING || sha256(JSON.stringify(bindingPayload)) !== REVIEWED_BINDING) {
  ...
  return { bundle, sources, allSources, fresh, claims: bundle.claims.map(claim => ({ ...claim,
