...
const StoredField = z.object({ value: z.string().min(1).max(80), unit: z.enum(['F', 'C', 'in']).nullable(),
  confidence: z.number().min(0.85).max(1), basis: z.literal('ocr_transcription'),
  source_media: z.tuple([z.string().regex(/^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/)]),
  prepared_image: z.object({ width: z.number().int().min(1).max(1800), height: z.number().int().min(1).max(1800), bbox: Bbox }).strict(),
}).strict();
export const StoredPrintedEvidence = z.object({
  reader_version: z.literal(PRINTED_READER_VERSION), outcome: z.enum(['readable', 'unreadable', 'unavailable']),
  evidence_id: z.string().regex(/^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/), image_sha256: z.string().regex(/^[a-f0-9]{64}$/).nullable(),
  fields: z.object({ thermostat_mode: StoredField.nullable(), fan_mode: StoredField.nullable(), setpoint: StoredField.nullable(),
    room_temperature: StoredField.nullable(), filter_nominal_dimensions: StoredField.nullable() }).strict(),
  gaps: z.array(z.object({ field: z.enum(['thermostat_mode', 'fan_mode', 'setpoint', 'room_temperature', 'filter_nominal_dimensions']),
    reason: z.enum(['not_legible', 'ambiguous', 'reader_unavailable']) }).strict()).max(5),
}).strict().superRefine((read, ctx) => {
  for (const key of PRINTED_FIELD_KEYS) {
    const field = read.fields[key];
    const gaps = read.gaps.filter(gap => gap.field === key);
    const box = field?.prepared_image;
    if ((field ? gaps.length !== 0 : gaps.length !== 1) || field && (field.source_media[0] !== read.evidence_id || !read.image_sha256 ||
      !box || box.bbox.x0 >= box.bbox.x1 || box.bbox.y0 >= box.bbox.y1 || box.bbox.x1 > box.width || box.bbox.y1 > box.height)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Reading identity, bounds or gap mismatch' });
    }
  }
  if ((read.outcome === 'readable') !== Object.values(read.fields).some(Boolean)) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Reading outcome mismatch' });
});
