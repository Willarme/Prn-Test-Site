...
grep -oiE "(you (did|made|got|survived|finished|nailed)|nice work|well done|good job|the hard part|that('| i)s over|worst of it|no more (questions|forms)|homework|assignment)"
...
Find every money token on the surface:
  grep -noE '\$\s?[0-9][0-9,.]*(\s*[-–—]\s*\$?[0-9][0-9,.]*)?|[0-9]+\s*(dollars|per hour|an hour|/hr)'
